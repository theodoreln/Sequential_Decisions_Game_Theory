...
def Policy_Function_Approximation (n, base_salary, base_education, expenses, education_rate, max_education, gamma, ret, proba) :        
    ...
    objective = gb.quicksum(gb.quicksum(((alpha - beta * j - gamma * i) - Best_dec[i,j-base_education])**2 for j in range(base_education, max_education+1)) for i in range(n))
    ...
    for i in range(n) :
        for j in range(base_education, max_education+1) :
            Best_dec_tuned[i,j-base_education] = round(alpha_opt - beta_opt * j - gamma_opt * i,0)
       
    return Best_dec_tuned 
...
if __name__ == "__main__":
    ...
    def Plotting(Best_dec1, Best_dec2):
        ...
        axs[0].set_yticklabels(np.arange(base_education, base_education + data1.shape[0]))
        ...
        axs[1].set_yticklabels(np.arange(base_education, base_education + data2.shape[0]))
    ...
    Best_dec = Policy_LP(n, base_salary, base_education, expenses, education_rate, max_education, gamma, ret, proba)
    Best_dec_tuned = Policy_Function_Approximation(n, base_salary, base_education, expenses, education_rate, max_education, gamma, ret, proba)
