...
def Policy_DP(n,base_salary, base_education, expenses, education_rate, max_education, gamma, ret, proba) :
    ...
    for i in range(n-1, -1, -1) :
        for j in range(max_education - base_education, -1, -1) :
            if i == n-1 :
                V_array[i,j] = max([ret(j+base_education,0), ret(j+base_education,1)])
            else :
                V_array[i,j] = max([ret(j+base_education,0) + gamma * sum([proba(l,j,0)*V_array[i+1,l] for l in range(j,max_education-base_education+1)]), 
